            ...
            <section class="my-20 mx-5" id="history">
                @foreach ($orders as $order)
                    <div class="history-container my-7 border border-gray-300 pt-9 rounded-lg">
                        ...
                        <svg class="my-9 max-md:my-3 w-full" xmlns="http://www.w3.org/2000/svg" width="1216"
                        ...
                        @if ($order->orderDetails && $order->orderDetails->isNotEmpty())
                            @foreach ($order->orderDetails as $orderDetail)
                                ...
                                <div class="flex justify-center my-4">
                                    <form action="{{ route('rate.product', $orderDetail->products->id) }}" method="POST"
                                        class="w-full md:w-1/2">
                                        @csrf
                                        <label for="rating" class="block text-lg font-semibold mb-2 text-gray-700">Beri
                                            Rating:</label>
