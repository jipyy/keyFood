...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const updateCounterAndTotal = (card) => {
            ...
            const savedQuantity = cart[cardId]?.quantity || 0;
            ...
            const saveToLocalStorage = () => {
                const cart = getCartData();
                cart[cardId] = { quantity: parseInt(quantityInput.value) || 0 };
                saveCartData(cart);
            };
        ...
        // Fungsi untuk memeriksa perubahan secara berkala
        const startPolling = () => {
            let previousCartData = getCartData();
            setInterval(() => {
                const currentCartData = getCartData();
                if (JSON.stringify(previousCartData) !== JSON.stringify(currentCartData)) {
                    previousCartData = currentCartData;
                    document.querySelectorAll('[data-counter-id]').forEach(card => {
                        updateCounterAndTotal(card);
                    });
                }
            }, 1000); // Polling interval: 1 detik
        };
